...
public final class BenefitsServer {
    ...
    static HttpServer runServer(final int port, final boolean waitForCtrlC) {
        ...
        httpServer.addListener(new NetworkListener("HTTP", "benefits.example.com", port));
        ...
        try {
            httpServer.start();
            System.out.println("Benefits App (Direct Access) - server running @ http://benefits.example.com:8014/benefitsApp/");
            System.out.println("Benefits App (Via OpenIG) - server running @ http://benefits-ig.example.com:9002/benefitsApp/");
            if (waitForCtrlC) {
